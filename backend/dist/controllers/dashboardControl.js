...
export function toFrontendJob(job, applicantsCount = 0, shortlistedCount = 0) {
    return {
        id: String(job._id),
        title: job.job_title,
...
export function toFrontendCandidate(applicant) {
    ...
    return {
        ...
        yearsExperience: Math.max(0, ...experience.map((item) => Number(item.years_of_experience ?? 0)), ...skills.map((item) => Number(item.years_of_experience ?? 0))),
        ...
        skills: {
            technical: skills.map((item) => String(item.name ?? item)).filter(Boolean),
            soft: [],
        },
        education: education
            .map((item) => String(item.degree ?? item.institution ?? item))
            .filter(Boolean),
...
export const listJobs = async (_req, res) => {
    ...
    res.json({
        jobs: jobs.map((job) => toFrontendJob(job, applicants.filter((applicant) => applicant.job_title === job.job_title).length, results.filter((result) => String(result.job_id) === String(job._id)).length)),
    });
};
export const getJob = async (req, res) => {
    ...
    const applicantsCount = await Applicant.countDocuments({ job_title: job.job_title });
...
const dashBoardControl = async (_req, res) => {
    try {
        const applicants = await Applicant.find().lean();
        const jobs = await Job.find().lean();
        const shortlisted = applicants.filter((applicant) => applicant.applicant_state === "Shortlisted").length;
        res.status(200).json({
            applicants: applicants.map(toFrontendCandidate),
            jobs: jobs.map((job) => toFrontendJob(job, applicants.filter((applicant) => applicant.job_title === job.job_title).length, shortlisted)),
            stats: {
                activeJobs: {
                    value: jobs.length,
                ...
                inScreening: {
                    value: applicants.filter((applicant) => applicant.applicant_state === "Queued").length,
